...
{
	// redimensionam array-ul de postari deoarece adaugam ceva
	post **aux = realloc(rt->post_arr->arr, (rt->post_arr->size + 1) *
						 sizeof(post *));
	DIE(!aux, MEM_ERR);
	rt->post_arr->arr = aux;

	// initializam noua postare
	...
	// initializam like-urile postarii noi create
...
post *get_parent(post *root, int id)
{
	...
	if (!root->events)
		return NULL;
	post *current = NULL;
	// daca nu am gasit, ne ducem la copii si cautam mai departe
...
void create_repost(retea *rt, char *name, char *id_post, char *id_repost)
{
	...
	// events e alocat si este totul pe NULL / 0
...
void print_reposts(post *repost) {
	...
	if (!repost->events->n_children)
		return;
	// printam recursiv in timp ce parcurgem toti copiii
...
void get_reposts(retea *rt, int id_p, int id_rp)
{
	...
	if (id_rp == -1) {
		...
		// fiind postare, printam recursiv de la copiii sai
		...
		// fiind repost, printam recursiv de la ea insasi
...
void like(retea *rt, char *user, int id_p, int id_rp)
{
	...
	if (id_rp == -1) {
		...
		// pentru postare, verificam starea actuala (daca are like sau nu)
		// apoi o actualizam
		if (!p->liked[user_id]) {
			p->liked[user_id] = 1;
			p->likes++;
		...
		// pentru repost, cautam mai intai repostul pornind de la postarea
		// originala si actualizam iarasi like-ul
		post *found_repost = get_parent(rt->post_arr->arr[idx], id_rp);
...
void get_likes(retea *rt, int id_p, int id_rp)
{
	...
	while (rt->post_arr->arr[idx]->id != id_p)
		idx++;
	post *root = rt->post_arr->arr[idx];
	// in functie de faptul daca e sau nu postare, afisam nr de likeuri
	if (id_rp == -1) {
...
{
	// daca gasim un nr de likeuri mai mare decat cel curent, actualizam
	...
	// vom face cautarea recursiv de la repostul curent
...
void get_common_parent(retea *rt, int id_p, int idx1, int idx2)
{
	...
	// incepem de la reposturile in sine
	post *p1 = get_parent(root, idx1);
	post *p2 = get_parent(root, idx2);

	// parcurgem in sus structura arborescenta memorand tatii
	while (p1) {
		v1[k1++] = p1->id;
	...
	// verificam care este primul tata comun (si reposturile originale
	// pot fi luate in considerare, deci testam separat pentru acestea)
	for (int i = 0; i < k1; i++)
		if (v1[i] == idx2) {
...
void delete(retea *rt, int id_p, int id_r)
{
	...
	while (rt->post_arr->arr[idx]->id != id_p)
		idx++;
	if (id_r == -1) {
		// daca stergem o postare, stergem recursiv incepand cu aceasta
		printf("Deleted \"%s\"\n", rt->post_arr->arr[idx]->title);
		free_post(&rt->post_arr->arr[idx]);
		// shiftam toate celelalte postari de la cea stearsa la stanga ca sa
		// "acoperim" golul
		for (int i = idx; i < rt->post_arr->size - 1; i++)
			rt->post_arr->arr[i] = rt->post_arr->arr[i + 1];
		rt->post_arr->size--;
		// redimensionam array-ul de postari
		if (rt->post_arr->size) {
		...
		// obtinem repostul care trebuie sters si parintele
		...
		// shiftam toate reposturile incepand de la cel sters la stanga ca sa
		// "acoperim" golul
		...
		// redimensionam vectorul de copii
