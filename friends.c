...
void add_friend(retea *rt, char *friend1, char *friend2)
{
	...
	// punem 1 pe pozitia i,j si j,i (graf neorientat)
...
void remove_friend(retea *rt, char *friend1, char *friend2)
{
	int idx1 = get_user_id(friend1);
	int idx2 = get_user_id(friend2);
	// punem 0 pe pozitia i,j si j,i (graf neorientat)
	...
	while (!q_is_empty(q)) {
		...
		for (int i = 0; i < MAX_PEOPLE; i++) {
			if (rt->db_matrix[v][i] && v != i) {
				// daca e neviziitat, il adaugam in coada si crestem distanta
				if (!viz[i]) {
					viz[i] = 1;
					d[i] = d[v] + 1;
					q_enqueue(q, &i);
...
void get_suggestions(retea *rt, char *user)
{
	...
	for (int i = 0; i < MAX_PEOPLE; i++) {
		// luam toti prietenii userului
		if (rt->db_matrix[id][i]) {
			// luam fiecare prieten al prietenului si testam daca are 1 sau 0
			for (int j = 0; j < MAX_PEOPLE; j++) {
				if (rt->db_matrix[i][j] && !rt->db_matrix[id][j] && j != id) {
					// cand gasim o sugestie retinem intr-un vector
					suggestion[j] = 1;
					ok = 1;
				}
			}
...
void get_common_friends(retea *rt, char *friend1, char *friend2)
{
	...
	// parcurgem in paralel lista de prieteni a celor doi useri si retinem
	// intr un vector prietenii comuni
	for (int i = 0; i < MAX_PEOPLE; i++) {
...
// functie care se ocupa cu nr. de prieteni ai unui user
...
void get_popular(retea *rt, char *user)
{
	...
	// luam toti prietenii userului si calculam nr de prieteni
	for (int i = 0; i < MAX_PEOPLE; i++) {
		if (rt->db_matrix[idx][i]) {
			name = get_user_name(i);
			nr = get_nr_of_friends(rt, name);
			// retinem numarul maxim si userul aferent
			if (nr > max) {
				poz = i;
				max  = nr;
			}
...
{
	// parsam comanda si ceilalti parametri
